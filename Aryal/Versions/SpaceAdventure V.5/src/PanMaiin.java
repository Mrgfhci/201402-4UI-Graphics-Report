...
public class PanMaiin extends JPanel implements ActionListener {
    ...
    public PanMaiin() {
        ...
        ActionListener updateTask = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
                collision();
                aste.update();   // update the (x, y) position
                space.moveX();
                repaint();
            }
        };
    ...
    public void collision() {
       // http://stackoverflow.com/questions/335600/collision-detection-between-two-images-in-java
        Rectangle rect1 = new Rectangle(craft.getX(), craft.getY(),
                50, 30);
        Rectangle rect2 = new Rectangle(aste.getX(), aste.getY(), 50, 30);
    ...
    public void paint(Graphics g) {
        ...
        g2d.drawImage(space.getImage(), space.getsX(), space.getY(), getWidth(), getHeight(), this);
        if (space.getX() == - 980) {
            space.updateX();
        } else if (space.getsX() == -980) {
            space.updatesX();
        }
        switch (craft.getX()) {
            case -1:
